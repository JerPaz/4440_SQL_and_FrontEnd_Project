from flask import (Flask, session, redirect, url_for, request, render_template, flash, Response)
import pymysql
import re
import os
import json
...
conn = pymysql.connect(host='localhost', user='root', db='cs4400spring2020', passwd='password')
...
def login():
    ...
    if request.method == 'POST':
    	sql = ("CALL login('{u}', {p});".format(u=request.form['username'], p=request.form['password']))
...
def register():
    ...
    if request.method == 'POST':
        ...
        sql = "CALL register('{u}', '{e}', '{f}', '{l}', '{p}', {b}, '{t}');".format(u=username, e=email, f=firstName, l=lastName, p=password, b=balance, t=userType)
...
@app.route('/customer_current_information', methods=['GET'])
...
def customer_order():
	...
	food_sql = "SELECT foodName, price FROM cs4400spring2020.MenuItem WHERE foodTruckName = '{ft}';".format(ft="BubbaGumps")
	c.execute(food_sql)
	...
	if request.method == 'POST':
		for i in range(len(food_dict_list)):
			food_name_input = str(request.form['purchase_quantity_input_{}'.format(i)])
			if request.form.get('food_name_input_{}'.format(i)):
				if (food_name_input != ''):
					try:
						...
						order_prices.append(int(food_name_input) * food_dict_list[i]['food_price'])
						order_dict_list.append({'order_food': food_dict_list[i]['food_name'], 'order_quantity': int(food_name_input)})
						send_order = True
					except:
	...
	if send_order and len(order_dict_list) > 0:
		...
		order_insert_sql = "INSERT INTO cs4400spring2020.Orders(date, customerUsername) VALUES ('{o_date}', '{cus_user}' );".format(o_date = "1999-06-25", cus_user = session['username'])
